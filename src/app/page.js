...
import { getUserCookie, setUserCookie } from "./cookie";
import { v4 as uuidv4 } from 'uuid';

export default function Home() {
  const usernameRef = useRef(null);
  const user = getUserCookie();
  const userFound = user && user.username && user.id;

  console.log("User found:", userFound, user);

  if (userFound) {
    redirect('/pool', RedirectType.replace);
  }
  ...
  return (
    (!userFound ? <div className="flex min-h-screen items-center justify-center bg-white shadow-lg">
